...
#
# Mirrors Princeton RUDICS SFTP accounts into a local server directory.
...
usage() {
    cat <<'EOF'
servercopy_princeton.zsh - Bathymetrix(TM) MERMAID operations
https://bathymetrix.com
...
Requirements:
  - Set internal `credentials_file` and `server` paths
  - MERMAID must be set in the environment.
  - lftp must be installed and available on PATH.

Credentials CSV format:
  - The first two lines are treated as headers and skipped.
  - Column 4 must contain the SFTP username.
  - Column 5 must contain the SFTP password.
  - Columns are expected to be simple comma-separated fields.
  - Quoted commas in fields are not supported.

Notes:
  - Downloads from rudics.thorium.cls.fr using SFTP on port 22.
  - Mirrors the remote working directory into $MERMAID/server_princeton/.
...
credentials_file="$MERMAID/passwords/princeton.csv"
server="$MERMAID/server_princeton/"
